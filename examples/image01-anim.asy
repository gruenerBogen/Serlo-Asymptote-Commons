...
real complement_slope_start = -3;
real complement_slope_end = .6;
...
real begin_angle = atan(complement_slope_start) + pi;
real end_angle = atan(complement_slope_end);

for(int i = 0; i <= n; ++i) {
  save();

  real complement_slope = tan(begin_angle * (1 - i/n) + end_angle * (i/n));
  write(i, 1-i/n, complement_slope);
...
a.movie(BBox(), loops=0, delay=anim_delay, options="-density 600");
